...
import time
from utils.token_handler import find_usb_and_read_token_linux
import os
import threading
...
def usb_watcher():
    print("USB Watcher başlatıldı...")
    last_state = set(os.listdir("/media/pi/"))

    while True:
        current_state = set(os.listdir("/media/pi/"))
        if current_state != last_state:
            print("USB değişikliği algılandı!")
            token_data = find_usb_and_read_token_linux()
            if token_data:
                token, _ = token_data
                print(f" Yeni Token Okundu: {token}")
            else:
                print("USB takıldı ama token dosyası bulunamadı.")
...
if __name__ == "__main__":
    # USB watcher thread'i başlat
    watcher_thread = threading.Thread(target=usb_watcher, daemon=True)
