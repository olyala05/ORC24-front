...
def login():
    ...
    if request.method == "POST":
        ...
        whoami_url = f"{base_url}/orc24/whoami"
...
def auto_login():
    ...
    session["client_name"] = "auto"
    session["client_role"] = "viewer"  # veya 'manager', 'technical' ihtiyaca göre
    session["access_token"] = token
    session["login_success"] = True
    session["login_time"] = datetime.utcnow().isoformat()
...
def logout():
    session.clear()
    session["client_role"] = "viewer"  # geçici olarak viewer gibi davran
    return redirect(url_for("dashboard.dashboard"))